...
/obj/structure/fuel_port/on_update_icon()
	...
	if(open)
		add_overlay(mutable_appearance(icon, "[icon_state]_open"))
		var/obj/item/tank/tank = locate_tank()
		if(tank)
			if(tank.color)
				add_overlay(mutable_appearance(icon, "[icon_state]_tank", tank.color))
			else
				add_overlay(mutable_appearance(icon, "[icon_state]_tank_orange"))
	else
		add_overlay(mutable_appearance(icon, "[icon_state]_closed"))
