...
/obj/machinery/nuclear_cylinder_storage/cannot_transition_to(state_path, mob/user)
	if(locked)
		return SPAN_NOTICE("You must unlock \the [src] first!")
	if(!open)
		return SPAN_NOTICE("You must open \the [src] first!")
	if(length(cylinders))
		return SPAN_NOTICE("You must remove all cylinders from \the [src] first!")
	return ..()
...
/obj/machinery/nuclear_cylinder_storage/handle_mouse_drop(atom/over, mob/user)
	if(over == user && open && !panel_open && length(cylinders))
		...
		if(do_after(user, interact_time, src) && open && length(cylinders))
			user.visible_message(
